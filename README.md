...
- **`skills/hedera-mirror-queries`** — how to query Hedera network data (balances, transactions, tokens, NFTs, HCS messages, contract results, staking, nodes, fees) via the free mirror node REST API or the typed `@hiero-enterprise/mirror` TypeScript package: choosing a route, the full data map, pagination/timestamp/filter patterns, unit and encoding gotchas, and reality-vs-spec caveats observed on mainnet.
